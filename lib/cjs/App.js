...
export function App() {
    ...
    var getUpdatedParlayValues = function (legs) {
        ...
        var _loop_1 = function (leg) {
            ...
            if (leg.betType === propField[4]) {
                var relevantPlayerFilter = relevantTeam.top_5.filter(function (player) { return player.name === leg.frontend_id.split("/")[0]; });
    ...
    var tasksReducer = function (tasks, action) {
        switch (action.type) {
            ...
            case "loadSharedSlip": {
                var startOfExpirationDate = new Date(action.expires_at);
