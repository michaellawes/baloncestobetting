...
import { evaluateLeg, exportAsImage, getOverUnderStyling, getParlayLegStyling, getParlayType, getPropTextWithRespectToScreenSize, getPropValue, getReadableDate, numberWithCommas, round5, } from "../../utils/Util";
import { progressBarWidth, propField } from "../../utils/Constants";
library.add(fas);
export function Parlay(props) {
    var _this = this;
    var parlay_id = props.parlay_id, created_at = props.created_at, frontend_is_active = props.frontend_is_active, legs = props.legs, total_odds = props.total_odds, payout = props.payout, wager = props.wager, is_winner = props.is_winner, liveTeamData = props.liveTeamData, setNotification = props.setNotification;
    var getLiveValue = function (leg) {
        if (leg.betType === propField[1]) {
            ...
            return parseFloat(liveTeamData.get(leg.team).get("".concat(playerName, "_score"))).toFixed();
        }
    };
    ...
    var getProgressBarWidth = function (leg) {
        if (leg.betType === propField[1]) {
            var teams = leg.frontend_id.split("/")[0].split(" v ");
            var roadTeamName = teams[0];
            var liveTotalPointsScored = leg.lastValue
                ? leg.lastValue
            ...
            var team = leg.team;
            var liveTeamScore = parseFloat(parseFloat(liveTeamData.get(team).get("live_score")).toFixed());
            ...
            var team = leg.team;
            var playerName = leg.frontend_id.split("/")[0];
            var liveTeamScore = parseFloat(parseFloat(liveTeamData.get(team).get("".concat(playerName, "_score"))).toFixed());
            var propTeamScore = parseFloat(getPropValue(leg.text));
            var propTeamScoreFull = parseFloat((propTeamScore * 1.2).toFixed(2));
            if (liveTeamScore >= propTeamScore) {
                propTeamScoreFull = liveTeamScore;
