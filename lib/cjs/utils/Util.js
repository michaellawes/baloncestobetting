...
export var getNewPropLineGivenDiscountAndType = function (type, value, original, live_total) {
    if (type === discountTypes[0]) {
        var oldProp = parseInt(parseFloat(original.text).toFixed());
        var percentDecrease = parseFloat(value) / 100;
        var newPropText = parseInt((oldProp * (1 - percentDecrease)).toFixed()) + 0.5;
        return {
        ...
        var oddsBoost_1 = parseInt(value);
        var overOddsIncrease = getRandomInteger(parseInt((oddsBoost_1 / 2).toFixed()), oddsBoost_1);
        var base = original.over_odds < 0 ? 100 : original.over_odds;
        var newOverOdds = base + overOddsIncrease;
        return {
            text: original.text,
...
export var createPlayerSpecials = function (weeklySlate) {
    var playerSpecials = [];
    var allPlayers = [];
    for (var _i = 0, weeklySlate_1 = weeklySlate; _i < weeklySlate_1.length; _i++) {
        var slate = weeklySlate_1[_i];
        allPlayers.push.apply(allPlayers, __spreadArray(__spreadArray([], slate.road.top_5, false), slate.home.top_5, false));
    }
    var allViablePlayers = allPlayers.filter(function (player) {
        return player.has_special_prop_today && !isPlayerLockedOut(player.last_game);
    });
    if (allViablePlayers.length > 0) {
        for (var i = 0; i < allViablePlayers.length; i++) {
            var index = i % discountTypes.length;
            var discountType = discountTypes[index];
            var discount = getDiscountFromType(discountType);
            var player = allViablePlayers[i];
            var playerClone = Object.assign({}, player);
            playerClone.discountType = discountType;
            playerClone.discount = discount;
            playerClone.prop_line = getNewPropLineGivenDiscountAndType(discountType, discount, player.prop_line, player.live_total);
...
export var exportAsImage = function (element, legs, imageFileName) { return __awaiter(void 0, void 0, void 0, function () {
    var currentPosition, canvas, image;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                currentPosition = legs.scrollTop;
                legs.scrollTop = 0;
                legs.style.maxHeight = "".concat(Math.min(window.innerHeight - parseFloat((window.innerHeight / 4).toFixed()), 890), "px");
                return [4 /*yield*/, html2canvas(element)];
            case 1:
                canvas = _a.sent();
                image = canvas.toDataURL("image/png", 1.0);
                return [4 /*yield*/, downloadImage(image, imageFileName)];
            case 2:
                _a.sent();
...
export var getAllPlayerLiveScores = function (matchups) {
    var allPlayerData = new Map();
    for (var _i = 0, matchups_1 = matchups; _i < matchups_1.length; _i++) {
        var matchup = matchups_1[_i];
...
export var getDiscountFromType = function (type) {
    if (type === discountTypes[0]) {
...
export var getFinalGameFormatted = function (lastGame) {
...
export var getIndividualLegResultForParlays = function (parlay) { return __awaiter(void 0, void 0, void 0, function () {
    var matchup_id, query_ids, _a, data, error, legDictionary, _i, _b, leg, legTokens, betType, legId, lastLiveValue;
            ...
            case 1:
                ...
                if (data) {
                    ...
                    for (_i = 0, _b = parlay.legs; _i < _b.length; _i++) {
                        leg = _b[_i];
                        legTokens = leg.frontend_id.split("/");
                        betType = legTokens[legTokens.length - 1];
                        legId = legTokens[0] + "/" + betType;
        ...
        switch (_c.label) {
            ...
            case 1:
                ...
                for (_i = 0, allParlayLegs_1 = allParlayLegs; _i < allParlayLegs_1.length; _i++) {
                    ...
                    if (leg["special_leg_type"] !== undefined &&
...
export var getPlayerNameIsTooLong = function (playerName, isSpecial) {
...
export var getPropTextWithRespectToScreenSize = function (leg, screenWidth) {
    ...
    if (betType !== propField[1]) {
        ...
        if (screenWidth < 500) {
            var teams = teamMatchupString.split(" v ");
            var roadTeam = teams[0].length > 15 ? teams[0].substring(0, 13) + "..." : teams[0];
            var homeTeam = teams[1].length > 15 ? teams[1].substring(0, 13) + "..." : teams[1];
            teamMatchupString = roadTeam + " v " + homeTeam;
        }
        return "".concat(teamMatchupString, " ").concat(leg.text);
    }
};
export var getRandomInteger = function (min, max) {
...
export var getTodayIsLastDay = function (lastGame) {
    var withRespectiveToTimezone = new Date(lastGame);
...
var getPlayerLastGame = function () { return __awaiter(void 0, void 0, void 0, function () {
    var _a, data, error;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0: return [4 /*yield*/, supabase
...
var getAllPlayerData = function () { return __awaiter(void 0, void 0, void 0, function () {
    var _a, data, error, typedData, playerLastGameData, playerToLastGame, _i, typedData_1, player;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0: return [4 /*yield*/, supabase
                    .from("fb_players")
                    .select("name, team, pos, status, fantasy_team_name, avg, games_left, has_special_prop_today")];
            case 1:
                _a = _b.sent(), data = _a.data, error = _a.error;
                if (error)
                    throw error;
                ...
                typedData = data;
                return [4 /*yield*/, getPlayerLastGame()];
            case 2:
                playerLastGameData = _b.sent();
                playerToLastGame = Object.assign.apply(Object, __spreadArray([{}], playerLastGameData.map(function (x) {
                    var _a;
                    return (_a = {}, _a[x.name] = x.last_game, _a);
                }), false));
                for (_i = 0, typedData_1 = typedData; _i < typedData_1.length; _i++) {
                    player = typedData_1[_i];
...
var getMatchupInformation = function (matchupId) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, data, error, typedData, distinctTeams, allTeamMetadata, teamToMetadata, _i, allTeamMetadata_1, teamMetadata, allPlayerMetadata, playerToMetadata, _b, allPlayerMetadata_1, playerMetadata, matchups, teamNameToTeam, _c, distinctTeams_1, teamName, metadata, teamNameToMatchupName, matchupToMatchupSchema, _d, matchups_2, matchup, teamNames, roadTeam, homeTeam, _e, typedData_2, propInfo, propIdAndBetType, propId, betType, matchup, teamNames, homeTeam, matchupName, matchup, matchupTeam, playerMetadata, matchupName, matchup, weeklySlate, _loop_1, _f, matchups_3, matchup;
    return __generator(this, function (_g) {
        switch (_g.label) {
            ...
            case 2:
                ...
                return [4 /*yield*/, getAllPlayerData()];
            case 3:
                ...
                for (_d = 0, matchups_2 = matchups; _d < matchups_2.length; _d++) {
                    matchup = matchups_2[_d];
                ...
                for (_e = 0, typedData_2 = typedData; _e < typedData_2.length; _e++) {
                    propInfo = typedData_2[_e];
                    ...
                    if (matchupToMatchupSchema.has(propId)) {
                        ...
                        if (teamNameToMatchupName.has(propId)) {
                            ...
                            else if (playerMetadata !== undefined &&
                ...
                weeklySlate = [];
                _loop_1 = function (matchup) {
                    var matchupPropSlate = matchupToMatchupSchema.get(matchup);
                    var roadLiveScore = matchupPropSlate.road.live_score;
                    var homeLiveScore = matchupPropSlate.home.live_score;
                    var roadName = matchupPropSlate.road.name;
                    var homeName = matchupPropSlate.home.name;
                    ...
                    if (getDaysSinceLastMonday() == 6) {
                        ...
                        var lastDayRoadGamesSorted = matchupPropSlate.road.top_5
                            .map(function (player) { return new Date(player.last_game).getTime(); })
                            .sort(function (a, b) { return a - b; })
                            .filter(function (time) { return time > today_1.getTime(); });
                        matchupPropSlate.road.first_last_game = lastDayRoadGamesSorted[0];
                        var lastDayHomeGamesSorted = matchupPropSlate.home.top_5
                            .map(function (player) { return new Date(player.last_game).getTime(); })
                            .sort(function (a, b) { return a - b; })
                            .filter(function (time) { return time > today_1.getTime(); });
                        matchupPropSlate.home.first_last_game = lastDayHomeGamesSorted[0];
                        matchupPropSlate.lastGame = Math.max(lastDayHomeGamesSorted[lastDayHomeGamesSorted.length - 1], lastDayRoadGamesSorted[lastDayRoadGamesSorted.length - 1]);
                    }
                    weeklySlate.push(matchupPropSlate);
                };
                for (_f = 0, matchups_3 = matchups; _f < matchups_3.length; _f++) {
                    matchup = matchups_3[_f];
                    _loop_1(matchup);
                }
...
export var getDailySlate = function (matchupId) { return __awaiter(void 0, void 0, void 0, function () {
    var matchupToMatchupSchema, slate, _i, slate_1, matchup, roadScore, homeScore;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, getMatchupInformation(matchupId)];
            case 1:
                matchupToMatchupSchema = _a.sent();
                slate = Array.from(matchupToMatchupSchema.values());
                for (_i = 0, slate_1 = slate; _i < slate_1.length; _i++) {
                    matchup = slate_1[_i];
                    roadScore = matchup.road.live_score > 0
                        ? matchup.road.live_score
                        : parseFloat(matchup.road.team_total.text);
                    homeScore = matchup.home.live_score > 0
                ...
                return [2 /*return*/, slate];
        }
    });
}); };
export var getTeamNameWithRespectToScreenSize = function (teamName) {
    if (window.innerWidth < 428) {
        return teamName.substring(0, 20) + "...";
    }
    return teamName;
};
export var isPlayerLockedOut = function (lastGame) {
    return new Date() >= new Date(lastGame);
};
export var isTeamScoreLockedOut = function (lastFirstGame) {
