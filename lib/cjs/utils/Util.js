...
export var getDaysSinceLastMonday = function () {
    ...
    var today = new Date();
    console.log("".concat(today.getDay(), " === ").concat(prevMonday.getDay()));
    if (today.getDay() === prevMonday.getDay()) {
        return 0;
    }
    console.log("".concat(((today.getDay() - 1) % 7) + 1));
...
* */
export var refactorDailySlate = function (data) {
    var distinctTeams = Array.from(new Set(data.map(function (prop_metadata) { return prop_metadata.main_prop_id; })));
    var matchups = Array.from(new Set(data.map(function (prop_metadata) {
    ...
    var teamNameToTeam = new Map();
    for (var _i = 0, distinctTeams_1 = distinctTeams; _i < distinctTeams_1.length; _i++) {
        var teamName = distinctTeams_1[_i];
